...
public class ServiceTest {
	...
	//log stream is used to log all upcoming connector events and print them after a test case

	private static UserAgentImpl testAgent;
	//test agent is the user agent used for calling the methods and is unlocked by a pass-phrase
	private static final String testPass = "adamspass";
	//the agent adam from the so called MockAgentFactory that ships with las2peer for exactly those testing cases

	private static final String mainPath = "template/";
	//main path is the service path we defined at the beginning of our main service class
	...
	public void startServer() throws Exception {
		...
		node.startService(new ServiceNameVersion(MoodleDataProxyService.class.getName(), "1.0.0"), "a pass");
	...
	public void testGet() {
		try {
			MiniClient client = new MiniClient();//simulated webserver
